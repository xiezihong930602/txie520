...
class FeishuProductLoader:
    ...
    def load_pending_products(self) -> List[Dict[str, Any]]:
        ...
        for rec in pending:
            ...
            # 关联模板
            template_ids = self._get_link_ids(rec, "模板表 3")
            if not template_ids:
                template_ids = self._get_link_ids(rec, "关联模板")
            template_name = self._get_template_name(template_ids[0]) if template_ids else ""

            # 关联店铺
            shop_ids = self._get_link_ids(rec, "店铺表 3")
            if not shop_ids:
                shop_ids = self._get_link_ids(rec, "关联店铺")
            shop_name = self._get_shop_name(shop_ids[0]) if shop_ids else ""
