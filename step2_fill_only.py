...
def fill_one(page, style_name, cat_path, size_category):
    ...
    # 滚：直接设置scroller.scrollTop，循环直到全部找到或滚到底
    checked = set()
    data_map = {str(r[0]): r[1:] for r in data_rows}
    remaining = set(data_map.keys())
    
    for i in range(80):  # 80次 × 150px = 12000px
        page.evaluate("""(p) => {
            const s = document.querySelector('.vue-recycle-scroller');
            if (s) { s.scrollTop = p; s.dispatchEvent(new Event('scroll', {bubbles: true})); }
        }""", i * 150)
        time.sleep(0.3)
        ...
        for item in items:
            ...
            if short in remaining:
                try:
                    item.evaluate("""(el, cols) => {
                        const ns = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set;
                        const cb = el.querySelector('.jx-checkbox__inner');
                        if (cb) cb.click();
