...
class RpaPublisherExecutor(BaseExecutor):
    ...
    def execute(self, product: Product, auto_close: bool = True, auto_submit: bool = True) -> dict:
        ...
        try:
            ...
            self._select_shop(product.shop_name or "Noble Boys")
    ...
    def _init_browser(self):
        ...
        if self.user_data_dir:
            ...
            self.context.add_init_script("Object.defineProperty(Event.prototype, 'isTrusted', {get: function(){return true}});")
    ...
    def _select_shop(self, shop_name: str):
        ...
        shop_form = dialog.locator(".jx-form-item").filter(has_text="店铺").first
        
        # 多种可能的选择器触发器，依次尝试
        trigger = None
        trigger_selectors = [
            shop_form.locator(".jx-cascader").first,
            shop_form.locator(".jx-select").first,
            shop_form.locator(".el-select").first,
            shop_form.locator(".el-cascader").first,
            shop_form.locator("input").first,
        ]
        ...
        if not trigger:
            # 兜底：直接点店铺文字右边的可点击区域
            print("  兜底定位店铺选择器...")
            shop_label = shop_form.locator(":text-is('店铺')").first
            trigger = shop_label.locator("xpath=../following-sibling::*//*[self::input or self::div[contains(@class,'select') or contains(@class,'cascader')]]").first
            ...
            for (const sel of selectors) {
                ...
                for (const n of nodes) {
                    ...
                    if (text === name || text.includes(name)) {
                        // 优先点checkbox
                        const cb = n.querySelector('input[type="checkbox"], .jx-checkbox__inner, .el-checkbox__inner');
                        if (cb) {
                            cb.click();
                        } else {
                            n.click();
    ...
    def _apply_template(self, template_name: str):
        ...
        if not template_name:
            raise Exception("模板名称为空，请检查飞书表格「关联模板」字段是否已选择模板")
        ...
        # 直接找可见的模板名称选项点击（不写死下拉class，避免页面class变化失效）
        template_item = self.page.get_by_text(template_name, exact=True).filter(visible=True).first
        ...
        if chosen:
            ...
            if sz_ok:
                ...
                try:
                    ...
                    time.sleep(2)
                    select_container = self.page.locator(".size-template-select-container").nth(index)
                # 点选择框旁边的"同步"按钮（在同一行父元素里找）
                try:
                    sync_clicked = self.page.evaluate(f"""(idx) => {{
                        const containers = document.querySelectorAll('.size-template-select-container');
                        ...
                        for (let depth = 0; depth < 5; depth++) {{
                            if (!parent) break;
                            const links = parent.querySelectorAll('span, a, div, button');
                    ...
                    if sync_clicked:
                        print(f"  已点击尺码表{index+1}旁边的同步按钮")
                    time.sleep(2)
                except Exception as e:
                    print(f"  点同步按钮异常: {e}")
                # 展开下拉选择刚创建的尺码表
